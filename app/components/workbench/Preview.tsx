...
export const Preview = memo(() => {
  ...
  // New Viewport State for quick toggles
  ...
  const handleViewportChange = (size: 'desktop' | 'mobile' | 'game') => {
    ...
    if (isDeviceModeOn) {
      setIsDeviceModeOn(false); // Turn off complex device mode when using quick toggles
    }
  };
  ...
  const toggleDeviceMode = () => {
    ...
    if (!isDeviceModeOn) {
      setViewportSize('desktop'); // Reset viewport size when turning on advanced device mode
    }
  };
  ...
  return (
    <div ref={containerRef} className={`w-full h-full flex flex-col relative`}>
      ...
      <div className="flex-1 border-t border-bolt-elements-borderColor flex flex-col overflow-hidden">
        {/* New Device Toolbar (Only shown when not in complex device mode) */}
        {!isDeviceModeOn && (
          <div className="flex justify-center gap-4 p-2 bg-gray-800 text-white text-sm z-10 w-full shrink-0">
            <button onClick={() => handleViewportChange('mobile')} className={viewportSize === 'mobile' ? 'text-blue-400 font-bold' : ''}>📱 Mobile</button>
            <button onClick={() => handleViewportChange('desktop')} className={viewportSize === 'desktop' ? 'text-blue-400 font-bold' : ''}>💻 Web</button>
            <button onClick={() => handleViewportChange('game')} className={viewportSize === 'game' ? 'text-blue-400 font-bold' : ''}>🎮 Game (16:9)</button>
          </div>
        )}

        <div className="flex-1 w-full flex justify-center items-center overflow-auto bg-bolt-elements-background-depth-1">
          <div
            style={{
              ...
              <>
                {isDeviceModeOn && showDeviceFrameInPreview ? (
                  ...
                  <iframe
                    ...
                    className={`transition-all duration-300 bg-white ${viewportSize === 'desktop' ? 'w-full h-full border-none' : ''}`}
                    src={iframeUrl}
                    sandbox="allow-scripts allow-forms allow-popups allow-modals allow-storage-access-by-user-activation allow-same-origin"
